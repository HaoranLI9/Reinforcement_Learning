...
class DDPG(object):
    ...
    def learn(self):
        ...
        #Ce réseau ne met pas à jour les paramètres à temps, et est utilisé pour prédire l'action dans Q_target de Critic
        q_ = self.Critic_target(bs_,a_)
        # Ce réseau ne met pas à jour les paramètres dans le temps, ce qui est utilisé pour donner la force de montée du gradient lorsque Actor met à jour les paramètres
        q_target = br+GAMMA*q_  # q_target <0
        #print(q_target)
        q_v = self.Critic_eval(bs,ba)
        #print(q_v)
        td_error = self.loss_td(q_target,q_v)
        # td_error=R + GAMMA * ct（bs_,at(bs_)）-ce(s,ba) Mettre à jour ce, mais ae(s) est le ba en mémoire, de sorte que le Q obtenu par ce soit proche de Q_target, de sorte que l'évaluation soit plus précise

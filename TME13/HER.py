...
class DQNAgent(object):
    def __init__(self, env, dim_state, opt):
        self.opt=opt
        self.env=env
    ...
    def make_batch(self, nb_samples):
        ...
        return state_batch.float(), action_batch, next_state_batch.float(), reward_batch.view(-1), done_batch.view(-1), idx_batch, w_batch
    ...
    def learn(self, nb_samples, gamma):
        ...
        with torch.no_grad():
            ...
            q_values_next = self.theta(next_state)

            next_q_value = next_q_values[torch.arange(start=0, end=q_values.size(0)), q_values_next.argmax(dim=1)]
...
if __name__ == '__main__':
    env, config, outdir, logger = init('./configs/config_random_gridworld.yaml', "DQNAgent")
    env.setPlan("gridworld/gridworldPlans&Goals/plan2Multi.txt")
    ...
    obs = env.reset()
    dim_state = featureEXtractor.getFeatures(obs).shape[1]


    agent = DQNAgent(env, dim_state, config)
    ...
    for i in range(episode_count):
        ...
        while True:
            ...
            ob = new_ob
            action = agent.act(np.concatenate([ob,goal], aixs=1), epsilon_by_frame(i+1))
            ...
            if ((config["maxLengthTrain"] > 0) and (not agent.test) and (j == config["maxLengthTrain"])) or ( (agent.test) and (config["maxLengthTest"] > 0) and (j == config["maxLengthTest"])):
                ...
                for state, action, state_, reward, done, k in ls_target:
                    agent.store(np.concatenate([state, goal_], axis=1), action, np.concatenate([state_, goal_], axis=1), -0.1, done, k)
                
                agent.store(torch.concatenate([ob, goal_], axis=1), action, torch.concatenate([goal_, goal_], axis=1), 1, 1, j)

                print("forced done!")"""

            agent.store(np.concatenate([ob, goal], axis=1), action, np.concatenate([new_ob, goal], axis=1), reward, done, j)
            ls_traject.append((ob, action, new_ob, reward, done, j))
            rsum += reward
